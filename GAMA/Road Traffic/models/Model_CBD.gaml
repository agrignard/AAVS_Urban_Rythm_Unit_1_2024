...
global {
	...
	file shape_file_roads <- file("../includes/GIS/cbd_roads.shp");
	file shape_file_bounds <- file("../includes/GIS/cbd_bounds.shp");
	geometry shape <- envelope(shape_file_bounds);
	float step <- 1 #sec;
	date starting_date <- date("2023-07-04-05-55-00");
	int nb_people <- 100;
	int min_work_start <- 2;
	...
	init {
		...
		create road from: shape_file_roads with:[type::string(read ("highway"))]{
			if (type!="footway"){
                do die;
			}
		}
		...
		list<building> industrial_buildings <- building  where (each.type="work") ;
